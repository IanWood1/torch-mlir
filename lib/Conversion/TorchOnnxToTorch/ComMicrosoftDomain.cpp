  ...
  patterns.onOp(
      ...
      [](OpBinder binder, ConversionPatternRewriter &rewriter) {
        ...
        if (hiddenSize % numHeads != 0)
          return rewriter.notifyMatchFailure(
              binder.op, "hidden size must be divisible by num_heads");
        ...
        // Share buffer: scatter directly into past buffer (no padding/growing).
        // The KV cache is pre-allocated to max_seq_len and reused across steps.
        // Present output has the same shape as past input.
        Value presentKey = pastKey;
        Value presentValue = pastValue;

        // Insert current K/V into the past buffer using scatter.src.
        // pastLen = seqlens_k + 1 - seq_len  (start position for new tokens)
        // In share buffer mode, the KV cache is pre-allocated to total_seq_len
        // and new tokens are inserted using per-batch index tensors.
        //
        // For dim=2 on a 4D tensor, scatter semantics are:
        //   self[i][j][index[i][j][k][l]][l] = src[i][j][k][l]
        //
        // So index[b][h][s][d] = pastLen[b] + s
        ...
        // Build per-batch scatter index: pastLen[b] + arange(seq_len)
        // offsets = arange(seq_len) -> shape [seq_len]
        Type si64Type = rewriter.getIntegerType(64, /*isSigned=*/true);
        Torch::ValueTensorType offsetsType =
            Torch::ValueTensorType::get(context, {sequenceLength}, si64Type);
        Value offsets = Torch::AtenArangeOp::create(
            rewriter, loc, offsetsType, cstSequenceLength, cstInt64Dtype,
            /*layout=*/cstNone, /*device=*/cstNone, /*pin_memory=*/cstNone);

        // View pastLen as [batch, 1, 1, 1] for broadcasting
        Value pastLenViewList = Torch::PrimListConstructOp::create(
            rewriter, loc, intListType,
            SmallVector<Value>{cstIntMinusOne, cstIntOne, cstIntOne,
                               cstIntOne});
        Torch::ValueTensorType pastLenViewType = Torch::ValueTensorType::get(
            context, SmallVector<int64_t>{batchSize, 1, 1, 1}, si64Type);
        Value pastLenView = Torch::AtenViewOp::create(
            rewriter, loc, pastLenViewType, pastLen, pastLenViewList);

        // View offsets as [1, 1, seq_len, 1] for broadcasting
        Value offsetsViewList = Torch::PrimListConstructOp::create(
            rewriter, loc, intListType,
            SmallVector<Value>{cstIntOne, cstIntOne, cstIntMinusOne,
                               cstIntOne});
        Torch::ValueTensorType offsetsViewType = Torch::ValueTensorType::get(
            context, SmallVector<int64_t>{1, 1, sequenceLength, 1}, si64Type);
        Value offsetsView = Torch::AtenViewOp::create(
            rewriter, loc, offsetsViewType, offsets, offsetsViewList);

        // scatter_idx = pastLen_view + offsets_view
        // -> broadcasts to [batch, 1, seq_len, 1]
        Torch::ValueTensorType scatterIdxSmallType =
            Torch::ValueTensorType::get(
                context, SmallVector<int64_t>{batchSize, 1, sequenceLength, 1},
                si64Type);
        Value scatterIdx =
            Torch::AtenAddTensorOp::create(rewriter, loc, scatterIdxSmallType,
                                           pastLenView, offsetsView, cstIntOne);

        // Expand to [batch, kv_num_heads, seq_len, head_size]
        ...
        Torch::ValueTensorType scatterIdxType = Torch::ValueTensorType::get(
            ...
            SmallVector<int64_t>{batchSize, kvNumHeads, sequenceLength,
                                 headSize},
            si64Type);
        Value scatterIdxExpanded =
            Torch::AtenExpandOp::create(rewriter, loc, scatterIdxType,
                                        scatterIdx, expandSizeList, cstFalse);

        // scatter.src current K/V into buffer along dim 2 (sequence dimension)
        presentKey = Torch::AtenScatterSrcOp::create(
            rewriter, loc, resultTypes[1], presentKey, cstDim2,
            scatterIdxExpanded, kRotary);
        presentValue = Torch::AtenScatterSrcOp::create(
            rewriter, loc, resultTypes[2], presentValue, cstDim2,
            scatterIdxExpanded, vInput);
