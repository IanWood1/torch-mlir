...
static Value createLinalgPayloadCalculationForElementwiseOp(
  ...
  if (auto gtScalar = dyn_cast<AtenGtScalarOp>(op)) {
    ...
    // TODO: `gtTensor` and `gtScalar` share similar code and can be called
  ...
  if (auto geScalar = dyn_cast<AtenGeScalarOp>(op)) {
    ...
    // TODO: The `AtenGeScalarOp` and `AtenGtScalarOp` share a lot of code
  ...
  if (auto rsub = dyn_cast<AtenRsubScalarOp>(op)) {
    ...
    Value other = convertScalarToDtype(b, loc, operands[1], dtype);
    Value alpha =
        convertScalarToDtype(b, loc, operands[2], dtype,
                             /*srcOriginalDtype=*/operands[2].getType(),
                             /*dstOriginalDtype=*/dtype);
    if (dtype.isa<mlir::FloatType>()) {
...
// a "linalg.generic-like" op for modeling a fused subgraph of
// numpy-broadcasted operands. Modeling elementwise ops that way is
// potentially useful to allow a more centralized reasoning about
// multiversioning. However a cost model will be needed for "pre-fusing"
// elementwise ops that way, as it can potentially be a pessimization. A mild
// extension of this pattern should work for such a general op.
